...
public class xmgAugmentedFramedImage : xmgAugmentedVisionBase
{
    ...
    void Start()
    {
            ...
            if (m_webcamTexture)
			{
                ...
                float fovx_radian = (float)videoParameters.CameraFOVX * 3.1415f / 180.0f;
				int success = xmgAugmentedVisionBridge.xzimgFramedImageInitialize(captureWidth, captureHeight, videoParameters.GetProcessingWidth(captureWidth), videoParameters.GetProcessingHeight(captureHeight), fovx_radian);
				if (success == 1)
                   Debug.Log("xzimgMarkerlessInitialize - success");
			}
#elif UNITY_ANDROID
            ...
            xmgAugmentedVisionBridge.xzimgFramedImageInitialize(videoOptions.resolution_mode, videoParameters.UseFrontal, fovx_radian, videoParameters.HighPrecision);
			...
			int status = xmgAugmentedVisionBridge.xzimgFramedImageInitialize(ref videoOptions, videoParameters.GetProcessingWidth(), videoParameters.GetProcessingHeight(), fovx_radian);
			if (status <= 0) Debug.Log("Initialization has failed!");
#endif
            LoadImages();
			DisableObjects();
		}
    ...
    void Update () 
	{
        ...
        int ret = xmgAugmentedVisionBridge.xzimgFramedImageDetect(ref m_image, visionParameters.FilterStrength, 0, 1, 1);
        m_myWebCamEngine.ApplyTexture();
#elif (UNITY_IPHONE)
		xmgAugmentedVisionBridge.xzimgFramedImageDetect(visionParameters.FilterStrength, 0, 1, 1);
    ...
    void LoadImages()
	{
		for (int i = 0; i < visionParameters.ObjectPivotLinks.Count; i++)
		{
			if (visionParameters.ObjectPivotLinks[i].Classifier)
			{
				...
				int success = xmgAugmentedVisionBridge.xzimgFramedImageAddTarget(bytesHandle.AddrOfPinnedObject(), asset.bytes.Length, visionParameters.ObjectPivotLinks[i].ObjectRealWidth);
				if (success == 1) 
